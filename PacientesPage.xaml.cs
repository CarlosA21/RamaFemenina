...
public PacientesPage()
{
    ...
    private async void BtnVerDonaciones_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            // Mostrar diálogo con las donaciones del paciente (página 1)
    ...
    private async Task MostrarDialogoDonacionesPaciente(Paciente paciente, int paginaActual = 1)
    {
        ...
        try
        {
            ...
            if (totalDonaciones > 0)
            {
                ...
                // Contenedor para la lista de donaciones (se actualizará dinámicamente)
                var listaDonacionesContainer = new StackPanel { Spacing = 8 };

                // Contenedor para paginación (se actualizará dinámicamente)
                var paginacionContainer = new StackPanel();

                // Función para actualizar la vista de donaciones según la página
                Action<int> ActualizarVistaDonaciones = null;
                ActualizarVistaDonaciones = (pagina) =>
                {
                    System.Diagnostics.Debug.WriteLine($"[VER-DONACIONES] Actualizando a página {pagina}");

                    // Limpiar contenido anterior
                    listaDonacionesContainer.Children.Clear();
                    paginacionContainer.Children.Clear();

                    // Obtener donaciones de la página actual
                    ...
                    if (totalPaginas > 1)
                    {
                        ...
                        // Event handlers - Solo actualizan el contenido, NO recrean el diálogo
                ...
                // Llamar a la función para mostrar la página inicial
            ...
            // Crear y mostrar el diálogo (solo una vez)
